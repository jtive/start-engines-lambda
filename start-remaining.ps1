...
foreach ($service in $services) {
    ...
    aws lambda invoke --function-name start-engines-lambda-dev --payload "file://$tempFile" --cli-binary-format raw-in-base64-out --region $region output.json | Out-Null
...
Write-Host "Wait 60 seconds for tasks to start, then check status" -ForegroundColor Yellow
